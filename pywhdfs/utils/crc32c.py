...
class crc32c(object):
  ...
  def update_bits(self, data, off, length):

    def rshift(val, n): return val>>n if val >= 0 else (val+0x100000000)>>n

    if type(data) != array or data.itemsize != 1:
      buf = array("B", data)
    else:
      buf = data
  ...
  def update_int(self, data):
    ...
    crc = self.crcValue
    crc = rshift(crc, 8) ^ T[T8_0_start + ((crc ^ data) & 0xff)]
    self.crcValue = crc
